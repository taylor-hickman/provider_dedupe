...
class TestFullPipeline:
    ...
    def test_configuration_integration(self, tmp_path: Path):
        ...
        config_data = {
            ...
            "max_iterations": 10,
            "blocking_rules": [
                {"rule": "l.npi = r.npi", "description": "NPI match"}
            ],
            "comparisons": [
                {
                    "column_name": "npi",
    ...
    def test_error_recovery_during_training(self, tmp_path: Path):
        ...
        with patch('provider_dedupe.core.deduplicator.Linker') as mock_linker_class:
            ...
            mock_linker.training.estimate_u_using_random_sampling.side_effect = Exception("Training failed")
            mock_linker.training.estimate_parameters_using_expectation_maximisation = Mock()
    ...
    def test_blocking_rules_effectiveness(self, tmp_path: Path):
        ...
        configs = [
            ...
            DeduplicationConfig(
                blocking_rules=[
                    {"rule": "l.npi = r.npi", "description": "NPI only"}
                ]
            ),
            # Name-based blocking
            DeduplicationConfig(
                blocking_rules=[
                    {"rule": "l.family_name = r.family_name", "description": "Last name only"}
                ]
            ),
        ]
