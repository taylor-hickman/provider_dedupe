...
class TestCLIIntegration:
    ...
    def test_analyze_command(self, tmp_path: Path):
        ...
        result = runner.invoke(
            cli, ["analyze", str(input_file), "--output-dir", str(output_dir)]
        )
    ...
    def test_dedupe_command_basic(self, mock_linker_class, tmp_path: Path):
        ...
        runner = CliRunner()
        result = runner.invoke(
            cli, ["dedupe", str(input_file), str(output_file), "--threshold", "0.9"]
        )
    ...
    def test_dedupe_command_with_config(self, mock_linker_class, tmp_path: Path):
        ...
        result = runner.invoke(
            cli,
            ["dedupe", str(input_file), str(output_file), "--config", str(config_file)],
        )
    ...
    def test_dedupe_command_with_report(self, mock_linker_class, tmp_path: Path):
        ...
        runner = CliRunner()
        result = runner.invoke(
            cli, ["dedupe", str(input_file), str(output_file), "--generate-report"]
        )
    ...
    def test_dedupe_command_file_not_found(self):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["dedupe", "nonexistent.csv", "output.csv"])
    ...
    def test_visualize_command_basic(self, tmp_path: Path):
        ...
        with patch("matplotlib.pyplot.figure"), patch("matplotlib.pyplot.hist"), patch(
            ...
            runner = CliRunner()
            result = runner.invoke(
                cli, ["visualize", str(results_file), "--output-dir", str(output_dir)]
            )
    ...
    def test_visualize_command_missing_dependencies(self, tmp_path: Path):
        ...
        with patch.dict("sys.modules", {"matplotlib": None, "matplotlib.pyplot": None}):
            runner = CliRunner()
            result = runner.invoke(cli, ["visualize", str(results_file)])

            # Should handle missing dependencies gracefully
            assert (
                "Visualization dependencies not installed" in result.output
    ...
    def test_cli_debug_logging(self, tmp_path: Path):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["--debug", "analyze", str(input_file)])
    ...
    def test_cli_log_file_option(self, tmp_path: Path):
        ...
        runner = CliRunner()
        result = runner.invoke(
            cli, ["--log-file", str(log_file), "analyze", str(input_file)]
        )
    ...
    def test_context_object_handling(self, tmp_path: Path):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["analyze", str(input_file)])
